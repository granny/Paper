...
import io.papermc.generator.utils.CodecUtil;
...
public record RegistryData(
    ...
    public static final Codec<RegistryData> CODEC = RecordCodecBuilder.create(instance -> instance.group(
        ...
        CodecUtil.IDENTIFIER.optionalFieldOf("serialization_updater_field").forGetter(RegistryData::serializationUpdaterField),
    ...
    public record Api(ClassNamed klass, Optional<ClassNamed> holders, boolean legacyEnum, Optional<String> registryField) {
        public Api(ClassNamed name) {
            this(name, Optional.of(name), false, Optional.empty());
        }

        public static final Codec<Api> DIRECT_CODEC = RecordCodecBuilder.create(instance -> instance.group(
            CodecUtil.CLASS_NAMED.fieldOf("class").forGetter(Api::klass),
            CodecUtil.CLASS_NAMED.optionalFieldOf("holders").forGetter(Api::holders),
            Codec.BOOL.optionalFieldOf("legacy_enum", false).deprecated(8).forGetter(Api::legacyEnum),
            Codec.STRING.optionalFieldOf("registry_field").forGetter(Api::registryField)
        ).apply(instance, Api::new));

        public static final Codec<Api> CLASS_ONLY_CODEC = CodecUtil.CLASS_NAMED.xmap(Api::new, Api::klass);
    ...
    public record Impl(ClassNamed klass, String instanceMethod) {
        ...
        public static final Codec<Impl> DIRECT_CODEC = RecordCodecBuilder.create(instance -> instance.group(
            CodecUtil.CLASS_NAMED.fieldOf("class").forGetter(Impl::klass),
            CodecUtil.IDENTIFIER.optionalFieldOf("instance_method", ConstantDescs.INIT_NAME).forGetter(Impl::instanceMethod)
        ).apply(instance, Impl::new));

        public static final Codec<Impl> CLASS_ONLY_CODEC = CodecUtil.CLASS_NAMED.xmap(Impl::new, Impl::klass);
    ...
    public record Builder(ClassNamed api, ClassNamed impl) {
        ...
        public static final Codec<Builder> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            CodecUtil.CLASS_NAMED.fieldOf("api").forGetter(Builder::api),
            CodecUtil.CLASS_NAMED.fieldOf("impl").forGetter(Builder::impl)
